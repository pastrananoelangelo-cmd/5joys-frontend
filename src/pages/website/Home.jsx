  ...
  return (
    <>
      ...
      <Section tone="surface">
        <SectionHeading
          tag="Why 5Joys"
          title="Five reasons customers keep coming back"
          align="center"
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          {reasons.map((r, i) => (
            <div key={r.title} className="fj-card p-5 text-center">
              <p
                className="fj-display text-2xl font-semibold"
                style={{ color: "var(--red)" }}
              >
                {i + 1}
              </p>

              <h3 className="fj-display font-semibold text-base mt-2">
                {r.title}
              </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-4">
          {quickLinks.map((q) => (
            <button
              key={q.key}
              onClick={() => go(q.key)}
              className="fj-card p-6 text-left"
            >
              <h3 className="fj-display font-semibold text-base">
                {q.title}
              </h3>
              ...
              <span
                className="mt-4 inline-block text-sm font-extrabold"
                style={{ color: "var(--red)" }}
              >
                View &rarr;
              </span>
